<?php
    include_once("config/constantss.php");
    ...
    if (isset($_POST['forgot_password'])) {
        ...
        $emailCheckQuery = "SELECT * FROM tbl_admin WHERE email = '$email'";
        ...
        if ($emailCheckResult) {
            ...
            if (mysqli_num_rows($emailCheckResult) > 0) {
                $code = rand(999999, 111111);
                $updateQuery = "UPDATE tbl_admin SET code = $code WHERE email = '$email'";
                ...
                if ($updateResult) {
                    ...
                    if (mail($email, $subject, $message, $sender)) {
                        ...
                        echo "<script>";
                        echo "alert(\" Failed while sending code!\");"; 
...
if(isset($_POST['verifyEmail'])){
    ...
    $verifyQuery = "SELECT * FROM tbl_admin WHERE code = $OTPverify";
    $runVerifyQuery = mysqli_query($conn, $verifyQuery);
    if($runVerifyQuery){
        if(mysqli_num_rows($runVerifyQuery) > 0){
            $newQuery = "UPDATE tbl_admin SET code = 0";
...
if(isset($_POST['changePassword'])){
    ...
    if (strlen($_POST['password']) < 8) {
        echo "<script>";
                    echo "alert(\" Use 8 or more characters with a mix of letters, numbers & symbols\");"; 
                    echo "window.history.back()";
            echo "</script>";
        ...
        if ($_POST['password'] != $_POST['confirmPassword']) {
            ...
            $updatePassword = "UPDATE tbl_admin SET password = '$password' WHERE email = '$email'";
